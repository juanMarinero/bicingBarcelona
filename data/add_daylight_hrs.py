...
def add_daylight_hrs(data):
    ...
    daylight_hrs = pd.Series(list(map(hours_of_daylight, daysArr)), index=daysArr, name='daylight_hrs').sort_index()
    data = data.join(daylight_hrs)

    import inspect # get functions name 
    if (data['daylight_hrs'].isnull().values.any()): print(f'Missing daylight hours. Function: {inspect.stack()[0][3]}')

    data.index = origIndex;
    
    if 0: plt.plot(daylight_hrs); plt.plot(data.index, data['daylight_hrs'].values)
