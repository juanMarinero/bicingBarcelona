...
def dataConstructor(data = get_data(), w = get_weather()):
    ...
    if smallDataFrames:
        ...
        try: data.drop(columns=['updateTime'], inplace=True)
        except:...   
    ...
    X = data.drop(columns=[*boolVars, *dummyVars, 
                           'latitude', 'longitude', 'nearbyStations','streetName','streetNumber'])#.copy()
